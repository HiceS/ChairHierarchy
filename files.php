...
//
$_CONFIG['main_title'] = "The Griffen";
...
//
$_CONFIG['hidden_dirs'] = array("lost+found", "owncloud", "temp", "Games");
...
// If set to true, you should specify some login as well (see below).
...
// Näiteks: $_CONFIG['login'] = array(array("user1", "pass1"), array("user2", "pass2"));
// Võimalik lehte kaitsta ka ainult üldise parooliga.
// Näiteks: $_CONFIG['login'] = array(array(null, "pass"));
...
// For example: $_CONFIG['login'] = array(array("username1", "password1", "user"), array("username2", "password2", "admin"));
...
// For example: $_CONFIG['login'] = array(array("username", "password", "admin"));
// Default: $_CONFIG['login'] = array();
//
$_CONFIG['login'] = array();
...
// They only apply to admin accounts, regular login can never perform these operations.
// Default:
...
//
// List of directories where login are allowed to upload. 
...
//
$_CONFIG['starting_dir'] = "mediadrive/";
...
class GateKeeper
{
	public static function init()
	{
		...
		if(count(EncodeExplorer::getConfig("login")) > 0)
			session_start();			
		else
			return;
	...
	{
		foreach(EncodeExplorer::getConfig("login") as $user)
		{
			if($user[1] == $userPass)
			{
	...
	public static function getUserStatus(){
		if(GateKeeper::isUserLoggedIn() == true && EncodeExplorer::getConfig("login") != null && is_array(EncodeExplorer::getConfig("login"))){
			foreach(EncodeExplorer::getConfig("login") as $user){
	...
	public static function showLoginBox(){
		if(!GateKeeper::isUserLoggedIn() && count(EncodeExplorer::getConfig("login")) > 0)
			return true;
		return false;
	}
}
